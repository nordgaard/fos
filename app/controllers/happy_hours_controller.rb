class HappyHoursController < ApplicationController
  ...
  def create
    ...
    @happy_hour = HappyHour.new(user_id: params[:user_id], location_id: params[:location][:location_id], hype_title: params[:hype_title], start_hour: params[:start_hour][:time_of_day_id], end_hour: params[:end_hour][:time_of_day_id], hype_description: params[:hype_description], day_of_week_id: params[:day_of_week][:day_of_week_id])
    if @happy_hour.save
